...
class DistillDataset(Dataset):
    def __init__(self,
        ...
        if os.path.exists(catched_file) is False or data_args.overwrite_cache:
            ...
            if data_args.dataset_name == "sst5":
                # sst5
                all_dataset = load_from_disk('datasets/sst5_dataset/sst5_dataset/validation')
                all_labels = ['very negative', 'negative', 'neutral', 'positive', 'very positive']
                self.all_dataset = hu_Dataset.from_dict({
                    'task': ['sst5'] * len(all_dataset),
                    'input': all_dataset['text'],
                    'output': all_dataset['label_text'],
                    'options': [all_labels] * len(all_dataset),
                    'seed': ['100'] * len(all_dataset),
                })
            elif data_args.dataset_name == "hellaswag":
                # HellaSwag
                all_dataset = load_dataset("datasets/HellaSwag")
                all_dataset = all_dataset['validation']
                all_dataset = all_dataset.map(lambda x: {**x, 'output': x['endings'][x['label']]})
                self.all_dataset = hu_Dataset.from_dict({
                    'task': ['hellaswag'] * len(all_dataset),
                    'input': all_dataset['ctx'],
                    'output': all_dataset['output'],
                    'options': all_dataset['endings'],
                    'seed': ['100'] * len(all_dataset),
                })
            elif data_args.dataset_name == "mrpc":
                # mrpc
                all_dataset = load_dataset("glue", 'mrpc')
                all_dataset = all_dataset['validation']
                all_labels = ["No", "Yes"]
                all_dataset = all_dataset.map(lambda x: {**x, 'output': all_labels[x['label']]})
                all_dataset = all_dataset.map(lambda x: {**x, 'input': 
                    "{sentence1} Can we say \"{sentence2}\"? ".format(
                        sentence1=x["sentence1"],
                        sentence2=x["sentence2"])
                })
                self.all_dataset = hu_Dataset.from_dict({
                    'task': ['mrpc'] * len(all_dataset),
                    'input': all_dataset['input'],
                    'output': all_dataset['output'],
                    'options': [all_labels] * len(all_dataset),
                    'seed': ['100'] * len(all_dataset),
                })
                ...
                all_dataset = concatenate_datasets([all_dataset['validation_matched'], all_dataset['validation_mismatched']])
                ...
                raise NotImplementedError("Dataset {} not implemented".format(data_args.dataset_name))
            
            # webqs
            # all_dataset = load_dataset("/data/syf/wzs/datasets/WebQs")
            # all_dataset = all_dataset['test']
            # all_dataset = all_dataset.map(lambda x: {**x, 'output': _target_to_answers(x["targetValue"])})
            # self.all_dataset = hu_Dataset.from_dict({
            #     'task': ['webqs'] * len(all_dataset),
            #     'input': all_dataset['utterance'],
            #     'output': [''] * len(all_dataset),
            #     'options': [''] * len(all_dataset),
            #     'seed': ['100'] * len(all_dataset),
            # })
            
            # smcalflow
            # all_dataset = load_dataset("/data/syf/wzs/datasets/SMCalFlow")
            # if (self.is_train):
            #     all_dataset = all_dataset['train']['turns']
            # else:
            #     all_dataset = all_dataset['validation']['turns']
            # datasets = []
            # for entry in all_dataset:
            #     for turn in entry:
            #         datasets.append({"input": turn['user_utterance']['original_text'], "output": [turn["lispress"]]})
            # self.all_dataset = hu_Dataset.from_dict({
            #     'task': ['smcalflow'] * len(datasets),
            #     'input': [d['input'] for d in datasets],
            #     'output': [d['output'] for d in datasets],
            #     'options': [''] * len(datasets),
            #     'seed': ['100'] * len(datasets),
            # })
        ...
        self.task_dict_is_clf[data_args.dataset_name] = 1
