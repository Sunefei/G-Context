...
# 设置环境变量
os.environ["WANDB_PROJECT"] = "ICL"  # change if needed
os.environ["WANDB_ENTITY"] = ""  # change to your wandb account
os.environ["WANDB_API_KEY"] = ""  # change to your api-key
...
class Inferencer:
    ...
    def init_model_dataloader(self, cfg):
        ...
        # 计算inferencer的efficiency
        # 假设 self.dataset_reader 是您的完整数据集
        full_dataset = self.dataset_reader

        # 定义要包含在子集中的样本索引，这里我们选择前 10 条数据
        subset_indices = list(range(10))

        # 使用 Subset 创建数据集的子集
        subset_dataset = Subset(full_dataset, subset_indices)
        small_test_dataset = self.dataset_reader[:10]
        dataloader = DataLoader(
            small_test_dataset,
            batch_size=1,
            shuffle=True,
            collate_fn=lambda x: x[0]  # 每个 batch 仅取单个样本
        )
        model = hu.instantiate(cfg.model_config.model).eval().to('cpu')
...
def main(cfg):
    ...
    print(f"开始时间:{time.ctime(end_time)}")
