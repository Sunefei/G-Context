...
class DenseRetriever:
    def __init__(self, cfg) -> None:
        ...
        if cfg.pretrained_model_path is not None:
            state_dict = torch.load(cfg.pretrained_model_path, map_location=torch.device('cpu'))
            # 初始化模型（通常 from_pretrained 会加载 state_dict，但此时我们用自己加载的 state_dict）
            model = BertModel.from_pretrained(model_path, state_dict=state_dict)
            self.model = BiEncoder.from_pretrained(cfg.pretrained_model_path, config=model_config, state_dict=state_dict))
        else:
