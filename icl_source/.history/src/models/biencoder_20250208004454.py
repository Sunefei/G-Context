...
class BiEncoder(PreTrainedModel):
    ...
    def encode(self, input_ids, attention_mask, encode_ctx=False, **kwargs):
        ...
        # 获取词汇表大小
        ctx_vocab_size = self.ctx_model.get_input_embeddings().num_embeddings
        question_vocab_size = self.question_model.get_input_embeddings().num_embeddings
        vocab_size = max(ctx_vocab_size, question_vocab_size)
        # 限制输入id的范围，防止超出 [0, vocab_size-1]
        input_ids = torch.clamp(input_ids, min=0, max=vocab_size-1)
        
        # 固定最大长度: 使用配置中的 max_length（例如3000），如果没有则使用默认值
        max_length = 512 #self.config.max_length if hasattr(self.config, 'max_length') else 512

        # 获取当前输入序列的长度（假设第二维为序列长度）
        current_length = input_ids.size(1)
        if current_length < max_length:
            # 填充不足部分
            pad_length = max_length - current_length
            # 尝试使用 tokenizer 的 pad_token_id，若不存在则默认使用 0
            pad_token_id = 0 #self.tokenizer.pad_token_id if hasattr(self, 'tokenizer') and self.tokenizer.pad_token_id is not None else 0
            padding_tensor = torch.full((input_ids.size(0), pad_length), pad_token_id, dtype=input_ids.dtype, device=input_ids.device)
            input_ids = torch.cat([input_ids, padding_tensor], dim=1)
            # 同时对 attention_mask 进行填充，填充值为 0（表示不关注）
            padding_mask = torch.zeros((attention_mask.size(0), pad_length), dtype=attention_mask.dtype, device=attention_mask.device)
