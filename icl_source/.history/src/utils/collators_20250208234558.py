...
class DataCollatorWithPaddingAndCuda:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> BatchEncoding:
        ...
        for feat in features:
            feat['input_ids'] = feat['input_ids'][:self.max_length]
            if 'attention_mask' in feat:
                feat['attention_mask'] = feat['attention_mask'][:self.max_length]
...
class IBNDataCollatorWithPadding:
    ...
    def __call__(self, samples: List[InputSample]) -> BatchEncoding:
        ...
        print(ctx_indices.shape)
        print(self.pair_wise)
        print(self.hard_neg_per_step)
        print("################")
        print(labels)
        input()
