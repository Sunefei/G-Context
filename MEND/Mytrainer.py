...
def custom_data_collator(features):
    ...
    # 处理 labels（如果有多个答案，保留所有）
    if "clf_label" in batch:
        batch["labels"] = batch["clf_label"]

    # 处理 input_ids，padding
    if "input_ids" in batch:
        batch["input_ids"] = pad_sequence([torch.tensor(x) for x in batch["input_ids"]], 
                                          batch_first=True, padding_value=0)

    if "attention_mask" in batch:
        batch["attention_mask"] = pad_sequence([torch.tensor(x) for x in batch["attention_mask"]], 
                                               batch_first=True, padding_value=0)
...
class MyTrainer(Trainer):
    ...
    #         kwargs["data_collator"] = custom_data_collator  
