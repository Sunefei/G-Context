...
em_eval_metric = evaluate.load("exact_match")
...
def main():
    ...
    # print(train_dataset.all_dataset[0])
    ...
    # add for generation
    training_args.predict_with_generate = True  # 添加 predict_with_generate 参数
    training_args.include_inputs_for_metrics = True
    training_args.include_for_metrics = ['input']  # 添加 include_for_metrics 参数
    trainer = myTrainer(
        model=model,
        args=training_args,
        ...
        # try use generation metrics
        # compute_metrics=build_rouge_metrics_fn("validation", teacher_tokenizer, metric = em_eval_metric),
        compute_metrics=build_compute_metrics_fn("validation"),
        ...
        for eval_task in task_list:
            for step, seed in enumerate(eval_seeds_list.split(",")):
                ...
                if len(test_dataset) == 0:
                    ...
                    continue
                # try use generation metrics
                # trainer.computer_metrics = build_rouge_metrics_fn(task, teacher_tokenizer, metric=em_eval_metric)
                trainer.compute_metrics=build_compute_metrics_fn("validation")
