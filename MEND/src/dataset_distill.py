...
from dataset_wrapper import process_sst5_dataset,process_swag_dataset,process_mrpc_dataset,process_mnli_dataset
...
class DistillDataset(Dataset):
    def __init__(self,
        ...
        if os.path.exists(catched_file) is False or data_args.overwrite_cache:
            ...
            elif mother_task_list[0] == 'sst5':
                # change the dataset to SST-5
                self.all_dataset = process_sst5_dataset(sst5_path = "./sst5_dataset", 
                                                        is_train = self.is_train, 
                                                        is_target_train = self.is_target_train)
            elif mother_task_list[0] == 'swag':
                # change the datset to Hellaswag
                self.all_dataset = process_swag_dataset(swag_path = "./hellaswag_dataset", 
                                                        is_train = self.is_train, 
                                                        is_target_train = self.is_target_train)
            elif mother_task_list[0] == 'mrpc':
                # change the dataset to MRPC
                self.all_dataset = process_mrpc_dataset(is_train = self.is_train, 
                                                        is_target_train = self.is_target_train)
            elif mother_task_list[0] == 'mnli':
                # change the dataset to MNLI
                self.all_dataset = process_mnli_dataset(is_train = self.is_train, 
                                                        is_target_train = self.is_target_train)
            else:
        ...
        # print("1", len(self.all_dataset))
        # print(is_eval, self.is_train)
        ...
        if self.is_train and self.is_target_train is False:
            ...
            if self.is_target_train:
                self.target_dataset = self.all_dataset
            # print("2",len(self.target_dataset))
    ...
    def basic_getitem(self, item, demo_count=-100):
        ...
        if demo_count > 0:
            for i in range(self.demo_batch_count):
                ...
                # print(t_demo_inputs_tokens_here)
                # print(t_demo_token_type_ids)
